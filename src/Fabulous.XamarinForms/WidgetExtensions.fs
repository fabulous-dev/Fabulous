...
type AdditionalViewExtensions =
    [<Extension>]
    static member inline center(this: WidgetBuilder<'msg, #IViewMarker>, ?expand: bool) =
    ...
    [<Extension>]
    static member inline centerHorizontal(this: WidgetBuilder<'msg, #IViewMarker>, ?expand: bool) =
        let options = match expand with None | Some false -> LayoutOptions.Center | Some true -> LayoutOptions.CenterAndExpand
        this.AddScalar(View.HorizontalOptions.WithValue(options))

    [<Extension>]
    static member inline centerVertical(this: WidgetBuilder<'msg, #IViewMarker>, ?expand: bool) =
        let options = match expand with None | Some false -> LayoutOptions.Center | Some true -> LayoutOptions.CenterAndExpand
        this.AddScalar(View.VerticalOptions.WithValue(options))
        
    [<Extension>]
    static member inline alignStartHorizontal(this: WidgetBuilder<'msg, #IViewMarker>, ?expand: bool) =
        let options = match expand with None | Some false -> LayoutOptions.Start | Some true -> LayoutOptions.StartAndExpand
        this.AddScalar(View.HorizontalOptions.WithValue(options))
        
    [<Extension>]
    static member inline alignStartVertical(this: WidgetBuilder<'msg, #IViewMarker>, ?expand: bool) =
        let options = match expand with None | Some false -> LayoutOptions.Start | Some true -> LayoutOptions.StartAndExpand
        this.AddScalar(View.VerticalOptions.WithValue(options))
        
    [<Extension>]
    static member inline alignEndHorizontal(this: WidgetBuilder<'msg, #IViewMarker>, ?expand: bool) =
        let options = match expand with None | Some false -> LayoutOptions.End | Some true -> LayoutOptions.EndAndExpand
        this.AddScalar(View.HorizontalOptions.WithValue(options))
        
    [<Extension>]
    static member inline alignEndVertical(this: WidgetBuilder<'msg, #IViewMarker>, ?expand: bool) =
        let options = match expand with None | Some false -> LayoutOptions.End | Some true -> LayoutOptions.EndAndExpand
        this.AddScalar(View.VerticalOptions.WithValue(options))
        
    [<Extension>]
    static member inline fillHorizontal(this: WidgetBuilder<'msg, #IViewMarker>, ?expand: bool) =
        let options = match expand with None | Some false -> LayoutOptions.Fill | Some true -> LayoutOptions.FillAndExpand
        this.AddScalar(View.HorizontalOptions.WithValue(options))
        
    [<Extension>]
    static member inline fillVertical(this: WidgetBuilder<'msg, #IViewMarker>, ?expand: bool) =
        let options = match expand with None | Some false -> LayoutOptions.Fill | Some true -> LayoutOptions.FillAndExpand
        this.AddScalar(View.VerticalOptions.WithValue(options))

    [<Extension>]
    static member inline centerTextHorizontal(this: WidgetBuilder<_, #ILabelMarker>) =
        this.AddScalar(Label.HorizontalTextAlignment.WithValue(TextAlignment.Center))

    [<Extension>]
    static member inline style(this: WidgetBuilder<'msg, #IViewMarker>, style: Style) =
        this.AddScalar(NavigableElement.Style.WithValue(style))

    [<Extension>]
    static member inline centerTextVertical(this: WidgetBuilder<_, #ILabelMarker>) =
        this.AddScalar(Label.VerticalTextAlignment.WithValue(TextAlignment.Center))
        
    [<Extension>]
    static member inline font(this: WidgetBuilder<_, #ILabelMarker>, ?size: double, ?namedSize: NamedSize, ?attributes: FontAttributes) =
        this.AddScalars([|
    ...
    [<Extension>]
    static member inline font(this: WidgetBuilder<_, #IButtonMarker>, value: NamedSize) =
        this.AddScalar(Button.FontSize.WithValue(Device.GetNamedSize(value, typeof<Xamarin.Forms.Button>)))

    [<Extension>]
    static member inline paddingLayout(this: WidgetBuilder<_, #ILayoutMarker>, value: float) =
        this.AddScalar(Layout.Padding.WithValue(Thickness(value)))

    [<Extension>]
    static member inline cascadeInputTransparent(this: WidgetBuilder<_, #ILayoutMarker>, value: bool) =
        this.AddScalar(Layout.CascadeInputTransparent.WithValue(value))

    [<Extension>]
    static member inline isClippedToBounds(this: WidgetBuilder<_, #ILayoutMarker>, value: bool) =
        this.AddScalar(Layout.IsClippedToBounds.WithValue(value))

    [<Extension>]
    static member inline onLayoutChanged(this: WidgetBuilder<'msg, #ILayoutMarker>, value: 'msg) =
        this.AddScalar(Layout.LayoutChanged.WithValue(value))

    [<Extension>]
    static member inline ignoreSafeArea(this: WidgetBuilder<_, #IPageMarker>) =
        this.AddScalar(AdditionalAttributes.iOS.UseSafeArea.WithValue(false))
        
    [<Extension>]
    static member inline androidToolbarPlacement(this: WidgetBuilder<_, #ITabbedPageMarker>, value: ToolbarPlacement) =
        this.AddScalar(AdditionalAttributes.Android.ToolbarPlacement.WithValue(value))
        
    [<Extension>]
    static member inline margin(this: WidgetBuilder<'msg, #IViewMarker>, value: float) =
        this.AddScalar(View.Margin.WithValue(Thickness(value)))
        
    [<Extension>]
    static member inline size(this: WidgetBuilder<'msg, #IViewMarker>, ?width: float, ?height: float) =
        this.AddScalars([|
    ...
    [<Extension>]
    static member inline padding(this: WidgetBuilder<_, #IViewMarker>, left: float, top: float, right: float, bottom: float) =
        this.AddScalar(Label.Padding.WithValue(Thickness(left, top, right, bottom)))
        
    [<Extension>]
    static member inline margin(this: WidgetBuilder<'msg, #IViewMarker>, left: float, top: float, right: float, bottom: float) =
        this.AddScalar(View.Margin.WithValue(Thickness(left, top, right, bottom)))

    [<Extension>]
    static member inline paddingLayout(this: WidgetBuilder<_, #ILayoutMarker>, left: float, top: float, right: float, bottom: float) =
