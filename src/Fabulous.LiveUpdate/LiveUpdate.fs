...
[<CLIMutable>]
type UpdateResponse = { Quacked: string }
                ...
                for i in 0 .. 3 do
                    try
                        ...
                        if iips.Length > 0 then
                            ...
                            printfn
                                "  See https://fsprojects.github.io/Fabulous/Fabulous.XamarinForms/tools.html for more details"
                    ...
                    with
                    | e -> printfn "LiveUpdate: error on broadcast: %A" e.Message
                    ...
                    try
                        ...
                        let! (resString: string) =
                            async {
                                ...
                                if (path = "/update") then
                                    let reader =
                                        new StreamReader(c.Request.InputStream, Encoding.UTF8)

                                    let! requestText = reader.ReadToEndAsync() |> Async.AwaitTask

                                    let req =
                                        Newtonsoft.Json.JsonConvert.DeserializeObject<(string * DFile) []>(requestText)
...
<html>
    <body>
        ...
        <p>in your project directory</p>
                        ...
                        do!
                            c.Response.OutputStream.WriteAsync(bytes, 0, bytes.Length)
                            |> Async.AwaitTask
            ...
            with
            | :? HttpListenerException as ex -> printfn "couldn't start listener %s" (ex.ToString())
        }
        |> Async.Start
...
module Extensions =
    ...
    let internal changeProgram (runner: ProgramRunner<'arg, 'model, 'msg>) syncChangeProgram programErased =
        ...
        syncChangeProgram (fun () -> runner.ChangeProgram(programErased))
            ...
            lock
                ...
                (fun () ->
                    ...
                    | Result.Error exn ->
                        ...
                        { Quacked =
                              sprintf
                                  "couldn't quack! the evaluation of the declarations in the code package failed: %A"
                                  exn }
                                    ...
                                    (fun (n, file) ->
                                        ...
                                        | Some (membDef, _) ->
                                            if membDef.Parameters.Length > 0 then
                                                ...
                                                printfn
                                                    "***   [x] found declaration called 'programLiveUpdate' or 'program'"

                                                printfn
                                                    "***   FAIL: the declaration has parameters, it must be a single top-level value"
                                                ...
                                                let entity =
                                                    interp(n).ResolveEntity(membDef.EnclosingEntity)

                                                let (_, programObj) =
                                                    interp(n).GetExprDeclResult(entity, membDef.Name)
