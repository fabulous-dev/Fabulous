...
type EnvironmentValue<'T> =
    ...
    member inline this.Current = this.Environment.Get(this.Key)

    member inline this.Set(value: 'T) = this.Environment.Set(this.Key, value)
...
module EnvironmentAttrs =
    let Environment =
        Attributes.defineSimpleScalar "Environment" ScalarAttributeComparers.noCompare (fun prevOpt currOpt node ->
...
type EnvironmentExtensions =
    ...
    static member inline Bind
        (
            ...
            [<InlineIfLambda>] value: Environment<'T>,
            [<InlineIfLambda>] continuation: EnvironmentValue<'T> -> ComponentBodyBuilder<'marker>
        ) =
