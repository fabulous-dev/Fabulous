...
type ComponentStateRequest<'T> = delegate of unit -> 'T
...
type StateExtensions =
    [<Extension>]
    static member inline Bind(_: ComponentBuilder, [<InlineIfLambda>] fn: ComponentStateRequest<'T>, [<InlineIfLambda>] continuation: ComponentState<'T> -> ComponentBodyBuilder<'msg, 'marker>) =
        ComponentBodyBuilder<'msg, 'marker>(fun bindings ctx ->
            ...
            let state = ComponentState(ctx, key, value)
            (continuation state).Invoke((bindings + 1<binding>), ctx)
        )
        
[<AutoOpen>]
module StateHelpers =
    let inline state value = ComponentStateRequest(fun () -> value)
