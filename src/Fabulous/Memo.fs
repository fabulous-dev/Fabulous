...
module Memo =
    ...
    let private widgetDefinition: WidgetDefinition =
        { Key = MemoWidgetKey
          ...
          CreateView =
            fun (widget, treeContext, envContext, parentNode) ->
                ...
                let struct (node, view) =
                    memoizedDef.CreateView(memoizedWidget, treeContext, envContext, parentNode)

                // store widget that was used to produce this node
                // to pass it to reconciler later on
                node.MemoizedWidget <- Some memoizedWidget
                struct (node, view)
          AttachView = fun (_widget, _treeContext, _envContext, _parentNode, _view) -> failwith "Memo widget cannot be attached" }
