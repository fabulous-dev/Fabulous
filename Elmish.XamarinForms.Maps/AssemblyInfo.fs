// Auto-Generated by FAKE; do not edit
namespace System
open System.Reflection

[<assembly: AssemblyTitleAttribute("Elmish.XamarinForms.Maps")>]
[<assembly: AssemblyProductAttribute("Elmish.XamarinForms.Maps")>]
[<assembly: AssemblyDescriptionAttribute("Elmish.XamarinForms bindings for Xamarin.Forms.Maps")>]
[<assembly: AssemblyVersionAttribute("0.11.0")>]
[<assembly: AssemblyFileVersionAttribute("0.11.0")>]
do ()

module internal AssemblyVersionInformation =
    let [<Literal>] AssemblyTitle = "Elmish.XamarinForms.Maps"
    let [<Literal>] AssemblyProduct = "Elmish.XamarinForms.Maps"
    let [<Literal>] AssemblyDescription = "Elmish.XamarinForms bindings for Xamarin.Forms.Maps"
    let [<Literal>] AssemblyVersion = "0.11.0"
    let [<Literal>] AssemblyFileVersion = "0.11.0"
