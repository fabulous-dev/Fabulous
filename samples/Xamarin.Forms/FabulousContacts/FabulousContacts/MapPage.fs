...
module MapPage =
    ...
    let view model =
        let map userPositionOpt pins =
            (Map(requestedRegion = MapSpan.FromCenterAndRadius(
                getUserPositionOrDefault userPositionOpt,
                Distance.FromKilometers(25.)
             )) {
                 for pin in pins do
                     Pin(pinType = pin.PinType, label = pin.Label, position = pin.Position)
                         .address (pin.Address)
             })
                .hasZoomEnabled(true)
        ...
        ContentPage(
            ...
            | None -> ContentView(centralLabel Strings.MapPage_Loading)
            | Some pins -> ContentView(map model.UserPosition pins)
        )
