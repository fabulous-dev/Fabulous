...
module App = 
        ...
        View.ContentPage(
          content=View.StackLayout(padding=30.0,verticalOptions = LayoutOptions.Center,
            children=[ 
              ...
              yield View.Label(automationId="CountLabel", text=sprintf "%d" model.Count, horizontalOptions=LayoutOptions.Center, widthRequest=200.0, horizontalTextAlignment=TextAlignment.Center)
              yield View.Button(automationId="IncrementButton", text="Increment", command= (fun () -> dispatch Increment))
              yield View.Button(automationId="DecrementButton", text="Decrement", command= (fun () -> dispatch Decrement)) 
              ...
              yield View.Slider(automationId="StepSlider", minimumMaximum=(0.0, 10.0), value= double model.Step, valueChanged=(fun args -> dispatch (SetStep (int (args.NewValue + 0.5)))))
              yield View.Label(automationId="StepSizeLabel", text=sprintf "Step size: %d" model.Step, horizontalOptions=LayoutOptions.Center) 
    ...
    let convertedInit () =
        let newModel, cmdMsgs = init ()
        let cmd = cmdMsgs |> List.map cmdMsgToCmd |> Cmd.batch
        newModel, cmd
    ...
    let program = 
        Program.mkProgram convertedInit convertedUpdate view 
