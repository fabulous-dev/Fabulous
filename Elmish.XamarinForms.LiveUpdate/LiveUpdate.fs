...
<html>
    <body>
        ...
        <pre>    %USERPROFILE%\.nuget\packages\Elmish.XamarinForms.LiveUpdate\0.17.0\tools\fscd.exe --watch --webhook:http://localhost:PORT/update</pre>
        <pre>    mono ~/.nuget/packages/Elmish.XamarinForms.LiveUpdate/0.17.0/tools/fscd.exe --watch --webhook:http://localhost:PORT/update</pre>
