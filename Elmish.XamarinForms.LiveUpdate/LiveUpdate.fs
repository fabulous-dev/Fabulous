...
<html>
    <body>
        ...
        <pre>    %USERPROFILE%\.nuget\packages\Elmish.XamarinForms.LiveUpdate\0.13.10\tools\fscd.exe --watch --webhook:http://localhost:PORT/update</pre>
        <pre>    mono ~/.nuget/packages/Elmish.XamarinForms.LiveUpdate/0.13.10/tools/fscd.exe --watch --webhook:http://localhost:PORT/update</pre>
