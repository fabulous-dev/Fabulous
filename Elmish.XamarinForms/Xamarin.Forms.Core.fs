...
module XamlElementExtensions = 
    ...
    type XamlElement with
        ...
        member x.Resources(value: seq<string * obj>) = x.WithAttribute("Resources", box ((value)))
...
type Xaml() =
    ...
    static member internal VisualElement(?anchorX: double, ?anchorY: double, ?backgroundColor: Xamarin.Forms.Color, ?heightRequest: double, ?inputTransparent: bool, ?isEnabled: bool, ?isVisible: bool, ?minimumHeightRequest: double, ?minimumWidthRequest: double, ?opacity: double, ?rotation: double, ?rotationX: double, ?rotationY: double, ?scale: double, ?style: Xamarin.Forms.Style, ?translationX: double, ?translationY: double, ?widthRequest: double, ?resources: seq<string * obj>, ?classId: string, ?styleId: string) = 
        ...
        let update (prevOpt: XamlElement voption) (source: XamlElement) (targetObj:obj) = 
            ...
            | ValueNone, ValueNone -> ()
            let prevValueOpt = match prevOpt with ValueNone -> ValueNone | ValueSome prev -> prev.TryGetAttribute<seq<string * obj>>("Resources")
            let valueOpt = source.TryGetAttribute<seq<string * obj>>("Resources")
