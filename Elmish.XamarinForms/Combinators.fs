...
type Update<'model, '_msg, 'msg> = '_msg -> 'model -> 'model * Cmd<'msg>
...
module Update =
    ...
    let combine2 (update1: _ -> Update<_,_,_>) (update2: _ -> Update<_,_,_>) : Update<_,_> = fun msg (model1, model2) ->
        match msg with
        | Choice1Of2 msg1 -> let newModel, cmds = update1 Choice1Of2 msg1 model1 in (newModel, model2), cmds 
        | Choice2Of2 msg2 -> let newModel, cmds = update2 Choice2Of2 msg2 model2 in (model1, newModel), cmds 

    let combine3 (update1: _ -> Update<_,_,_>) (update2: _ -> Update<_,_,_>) (update3: _ -> Update<_,_,_>) : Update<_,_> = fun msg (model1, model2, model3) ->
        match msg with
        | Choice1Of3 msg1 -> let newModel, cmds = update1 Choice1Of3 msg1 model1 in (newModel, model2, model3), cmds 
        | Choice2Of3 msg2 -> let newModel, cmds = update2 Choice2Of3 msg2 model2 in (model1, newModel, model3), cmds 
        | Choice3Of3 msg3 -> let newModel, cmds = update3 Choice3Of3 msg3 model3 in (model1, model2, newModel), cmds 

    let combine4 (update1: _ -> Update<_,_,_>) (update2: _ -> Update<_,_,_>) (update3: _ -> Update<_,_,_>) (update4: _ -> Update<_,_,_>) : Update<_,_> = fun msg (model1, model2, model3, model4) ->
        match msg with
        | Choice1Of4 msg1 -> let newModel, cmds = update1 Choice1Of4 msg1 model1 in (newModel, model2, model3, model4), cmds 
        | Choice2Of4 msg2 -> let newModel, cmds = update2 Choice2Of4 msg2 model2 in (model1, newModel, model3, model4), cmds 
        | Choice3Of4 msg3 -> let newModel, cmds = update3 Choice3Of4 msg3 model3 in (model1, model2, newModel, model4), cmds 
        | Choice4Of4 msg4 -> let newModel, cmds = update4 Choice4Of4 msg4 model4 in (model1, model2, model3, newModel), cmds 

    let combine5 (update1: _ -> Update<_,_,_>) (update2: _ -> Update<_,_,_>) (update3: _ -> Update<_,_,_>) (update4: _ -> Update<_,_,_>) (update5: _ -> Update<_,_,_>) : Update<_,_> = fun msg (model1, model2, model3, model4, model5) ->
        match msg with
        | Choice1Of5 msg1 -> let newModel, cmds = update1 Choice1Of5 msg1 model1 in (newModel, model2, model3, model4, model5), cmds
        | Choice2Of5 msg2 -> let newModel, cmds = update2 Choice2Of5 msg2 model2 in (model1, newModel, model3, model4, model5), cmds
        | Choice3Of5 msg3 -> let newModel, cmds = update3 Choice3Of5 msg3 model3 in (model1, model2, newModel, model4, model5), cmds
        | Choice4Of5 msg4 -> let newModel, cmds = update4 Choice4Of5 msg4 model4 in (model1, model2, model3, newModel, model5), cmds
        | Choice5Of5 msg5 -> let newModel, cmds = update5 Choice5Of5 msg5 model5 in (model1, model2, model3, model4, newModel), cmds
