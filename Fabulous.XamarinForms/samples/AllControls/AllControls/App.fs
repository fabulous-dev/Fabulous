    ...
    let getSampleDefinitionFromNode (node: Node) =
        ...
        | SampleChooser sampleChooser ->
            { Title = sampleChooser.Title
              ...
              MapToCmd = SampleChooser.mapToCmd }
            |> boxSampleDefinition
...
module App =
    ...
    type Model = { SampleStates: SampleState list }

    let displayMemoryWarningMessage () =
        Application.Current.MainPage.DisplayAlert("Low memory!", "Cleaning up data...", "OK")
        |> ignore

    let init () =
        let definition =
            SampleHelpers.getSampleDefinitionFromNode Samples.root

        let sampleModel, sampleCmdMsgs = definition.Init()

        let newCmd =
            sampleCmdMsgs
            |> List.map (definition.MapToCmd >> (Cmd.map SampleMsg))
            |> Cmd.batch
    ...
    let mapExternalMsg (externalMsg: obj option) =
        ...
        | Some (:? SampleChooser.ExternalMsg as msg) ->
            match msg with
            | SampleChooser.ExternalMsg.NavigateToNode node -> Cmd.ofMsg (NavigateToRequested node)
        | _ -> Cmd.none
    ...
    let update msg model =
        ...
        | SampleMsg sampleMsg ->
            ...
            | [] -> model, []
            | sampleState :: rest ->
                let newSampleModel, newCmdMsgs, externalMsg =
                    sampleState.Definition.Update sampleMsg sampleState.Model

                let newSampleState =
                    { sampleState with
                          Model = newSampleModel }

                let newCmd =
                    newCmdMsgs
                    |> List.map (
                        sampleState.Definition.MapToCmd
                        >> (Cmd.map SampleMsg)
                    )
                    |> Cmd.batch
                ...
                { model with
                      SampleStates = newSampleState :: rest },
                Cmd.batch [ newCmd; newExternalCmd ]

        | NavigateToRequested node ->
            let definition =
                SampleHelpers.getSampleDefinitionFromNode node
            ...
            { model with
                  SampleStates = sampleState :: model.SampleStates },
            newCmd

        | NavigationPopped ->
            { model with
                  SampleStates = model.SampleStates.Tail },
            Cmd.none
        ...
        | AppThemeChanged appTheme ->
            ...
            | sampleState :: _ ->
                ...
                | :? Samples.UseCases.AppTheming.Model ->
                    let appThemeMsg =
                        AllControls.Samples.UseCases.AppTheming.Msg.SetRequestedAppTheme appTheme
...
type App() as app =
    ...
    let onRequestedThemeChanged =
        EventHandler<AppThemeChangedEventArgs>
            (fun _ args -> runner.Dispatch(App.Msg.AppThemeChanged args.RequestedTheme))
