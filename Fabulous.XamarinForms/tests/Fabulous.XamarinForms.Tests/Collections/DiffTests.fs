...
module DiffTests =
    let testUpdateChildren prev curr =
        ...
        let workingSet = Array.zeroCreate<Operation<ViewElement>> (prevArrayLength + curr.Length)
        
        let workingSetLength = Collections.diff<ViewElement> true prevArrayLength prevArray currArray (fun v -> v.TryGetKey()) (fun prev curr -> canReuseView prev curr) workingSet
